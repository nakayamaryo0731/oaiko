...
- [x] ビジネス情報の登録（Stripeダッシュボード → Settings → Business details）
- [x] 銀行口座の登録（入金先）
- [x] 本番モード（Live mode）が有効化されていることを確認

### 2. 商品・価格の作成（Stripeダッシュボード Live mode）

- [x] 商品作成: "Pairbo Premium"
- [x] 価格作成: 月額 ¥300（JPY, recurring/monthly）
- [x] 価格作成: 年額 ¥2,400（JPY, recurring/yearly）
- [x] 各Price IDを控える（`price_xxx`形式）

### 3. Customer Portal設定

- [x] Stripeダッシュボード → Settings → Customer portal
- [x] 解約を許可
- [x] プラン変更を許可（月↔年の切り替え）

### 4. Webhook設定

- [x] Stripeダッシュボード → Developers → Webhooks → Add endpoint
- [x] エンドポイントURL: `https://<convex-deployment>.convex.site/stripe/webhook`
- [x] 以下のイベントを選択:
  - `checkout.session.completed`
  - `customer.subscription.updated`
  - `customer.subscription.deleted`
  - `invoice.payment_failed`
- [x] Webhook Signing Secretを控える（`whsec_xxx`形式）

### 5. Convex環境変数設定（本番デプロイメント）

- [x] 設定済み（2026-03時点で本番稼働確認済み）

### 6. 動作確認

- [x] 実ユーザー（オーナー）がPremiumプランに課金済みで動作確認完了
