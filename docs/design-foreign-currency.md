...
| **B. API自動取得 + 手動修正可能** | UX良好、正確、柔軟 | 外部API依存、UIが複雑                |
| **C. API自動取得のみ（採用）**    | シンプル、UX良好   | 実際のレートと乖離する場合に対応不可 |

→ **C案**を採用。APIで参考レートを自動取得し、手動修正UIは設けない。UIをシンプルに保つことを優先する。
...
- 外貨を選択すると、**為替レートが自動取得される**
- 手動でのレート修正UIは設けない（UIのシンプルさを優先）

#### 3. 為替レート自動取得

- **Frankfurter API**（ECBデータ）から日次レートを取得
- レートはConvex DBにキャッシュし、同日の再リクエストを回避
- APIが利用不可の場合はエラー表示（手動入力フォールバックなし）
...
- 可用性: 為替レートAPIがダウンした場合はエラー表示
- プラン: 全ユーザー利用可能（Premium限定にしない）
...
- **ユーザー操作**: 過去の支出を入力する場合も最新レートが自動セットされる（手動修正UIなし）
...
flowchart TD
    ...
    E -->|はい| F[換算額をリアルタイム表示]
    E -->|いいえ| G[エラー表示]
    F --> I[baseAmount = amount × exchangeRate]
...
| ファイル         | 変更内容                                                           |
| ---------------- | ------------------------------------------------------------------ |
| 支出入力フォーム | 通貨選択UI、レート自動取得、リアルタイム換算額表示（手動修正なし） |
...
> **ステータス: 保留（2026-04-03）**
> 以下の未解決の懸念があるため、実装を一旦保留している。

### 1. 小数対応（未解決）

- **懸念**: 現在の金額入力は整数（JPY前提）。USD等は小数2桁（$15.50）が必要
- **影響範囲**:
  - フロントエンドの `inputMode="numeric"` + `pattern="[0-9]*"` が整数前提
  - `splitCalculator` の `Math.floor` がJPY前提の丸め
  - 通貨ごとの `decimalDigits` に応じた入力制御・丸め処理が必要
- **対策案**: 通貨の `decimalDigits` に応じて入力制御と丸めロジックを分岐

### 2. API失敗時の支出記録（未解決）

- **懸念**: 手動レート修正UIを廃止したため、API失敗時に外貨支出が一切記録できない
- **影響**: 旅行中のネット不安定時に支出が記録できないのは致命的
- **対策案**:
  - A. レートなしでも記録し、後からレートを補完できるようにする
  - B. API失敗時のみ手動入力にフォールバック
  - C. 一旦ベース通貨で記録し、後から通貨を変更できるようにする

### 3. ベース通貨変更時の既存データ（未解決）

- **懸念**: グループのベース通貨をJPY→USDに変更した場合、既存の `expenseSplits.amount` はJPYのまま
- **対策案**:
  - A. ベース通貨変更を禁止する
  - B. 変更は許可するが過去データはそのまま（UIで注意表示）
  - C. 変更時に全支出を再計算する（実装コスト大）

### 4. splitCalculator の通貨対応（未解決）

- **懸念**: 現在の `Math.floor` はJPY（整数）前提。ベース通貨がUSD（小数2桁）の場合、丸め単位が変わる
- **対策案**: `decimalDigits` に応じた丸め関数を用意（`roundToDecimal(amount, digits)`）

### 5. ECBレートと実際のレートの乖離

- **懸念**: ECBの参考レートはクレジットカード決済レートや両替所レートと異なる
- **対策**: 手動修正UIを廃止したため、ユーザーはECBレートを受け入れるしかない。家計簿としては概算で十分と割り切る
